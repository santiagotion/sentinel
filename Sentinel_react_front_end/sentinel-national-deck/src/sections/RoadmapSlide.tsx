import { Calendar, Target, Rocket, Users, Database, Smartphone, Brain } from 'lucide-react';
  ...
  const roadmap = [
    {
      ...
      icon: Database,
      capacity: "Millions de messages/jour"
    },
    {
      year: "Année 2",
      budget: "2,5M $",
      ...
      icon: Brain,
      capacity: "Analyse multimédia complète"
    },
    {
      year: "Année 3",
      budget: "3,0M $",
      ...
      icon: Smartphone,
      capacity: "Communication directe citoyens"
    }
  ];

  // Visual representation of capacity growth
  const CapacityGrowthVisualization = () => {
    return (
      <div className="bg-gradient-to-r from-blue-50 to-emerald-50 dark:from-blue-900/10 dark:to-emerald-900/10 rounded-xl p-6 mb-8">
        <h4 className="text-lg font-semibold mb-6 text-center">Évolution des Capacités SENTINEL</h4>
          ...
          <div className="flex items-end justify-around h-32 mb-4">
            {roadmap.map((phase, index) => (
              <div key={index} className="flex flex-col items-center">
                <motion.div
                  initial={{ height: 0 }}
                  animate={{ height: `${30 + index * 30}px` }}
                  transition={{ delay: index * 0.5, duration: 1 }}
                  className={`w-16 bg-gradient-to-t ${phase.color} rounded-t-lg mb-2 flex items-end justify-center pb-2`}
                >
                  <phase.icon className="w-6 h-6 text-white" />
                </motion.div>
                <p className="text-sm font-medium">{phase.year}</p>
                <p className="text-xs text-gray-600 dark:text-gray-400">{phase.budget}</p>
              </div>
            ))}
          </div>
          
          {/* Growth curve */}
          <svg className="absolute top-0 left-0 w-full h-32 pointer-events-none">
            <motion.path
              d="M 50 100 Q 150 80 250 60 T 450 20"
              stroke="#059669"
              strokeWidth="3"
              fill="none"
              strokeDasharray="5,5"
