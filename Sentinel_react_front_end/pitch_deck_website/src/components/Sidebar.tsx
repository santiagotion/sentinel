...
interface SidebarProps {
  slides: string[];
  currentSlide: number;
  currentView: 'slides' | 'youtube';
  onNavigate: (index: number) => void;
  onViewChange: (view: 'slides' | 'youtube') => void;
}

export function Sidebar({ slides, currentSlide, currentView, onNavigate, onViewChange }: SidebarProps) {

  return (
    <>
      {/* Tablet Sidebar - Numbers Only */}
      <aside className="hidden md:block lg:hidden w-20 bg-gray-50 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 h-screen overflow-y-auto pb-24">
        <div className="p-4">
          ...
          <nav className="space-y-2">
            {slides.map((_, index) => (
              <motion.button
                ...
                whileHover={{ scale: currentView === 'slides' && currentSlide === index ? 1 : 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
            ...
            <motion.button
              ...
              className={`w-full py-3 rounded-lg transition-all duration-200 mt-4 ${
                currentView === 'youtube'
                  ? 'bg-congo-blue text-white font-bold shadow-lg'
                  : 'text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-800'
              }`}
              whileHover={{ scale: currentView === 'youtube' ? 1 : 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
      ...
      <aside className="hidden lg:block w-80 bg-gray-50 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 h-screen overflow-y-auto pb-24">
        <div className="p-6">
        ...
        <nav className="space-y-1">
          {slides.map((name, index) => (
            <motion.button
              ...
              whileHover={{ x: currentView === 'slides' && currentSlide === index ? 0 : 4 }}
              whileTap={{ scale: 0.98 }}
            >
          ...
          <motion.button
            ...
            whileHover={{ x: currentView === 'youtube' ? 0 : 4 }}
            whileTap={{ scale: 0.98 }}
          >
            <div className="flex items-center justify-between">
              <span className="flex items-center space-x-3">
